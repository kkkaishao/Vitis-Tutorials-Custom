...
/************************** RTP Configurations  *****************************/
  ...
  //{portId, aliasId, portName, aliasName, graphId, isInput, isAsync, isConnect, elemType, numBytes, isPL, hasLock, blocking , isSlaveRTP, selectorColumn, selectorRow, selectorAddr, selectorLockId, pingColumn, pingRow, pingAddr, pongColumn, pongRow, pongAddr, pongLockId, plKernelInstanceName, plParameterIndex, plDriverWriteRTP, plDriverReadRTP}
    {3, 0, "gr.k.in[1]", "gr.value", 0, true, true, false, (adf::RTPConfig::elementType)4, 1024, false, true, false, false, 8, 1, 0x3fe0, 2, 8, 0, 0, 0, 8, 0, 0x4000, 1, "", -1, nullptr, nullptr},
  };
  const int NUM_RTP = 1;
...
// Kernel Stub Definition
  template<int32 num> void vect_add(adf::input_buffer<int32, adf::extents<num>> &__restrict in, adf::output_buffer<int32, adf::extents<num>> &__restrict out, const int32 (&value)[num]) { /* Stub */ } 
  template void vect_add<256>(adf::io_buffer<int, adf::direction::in, adf::io_buffer_config<adf::extents<256>>> &__restrict,adf::io_buffer<int, adf::direction::out, adf::io_buffer_config<adf::extents<256>>> &__restrict,const int (&)[256]); /* explicit template instantiation */ 
